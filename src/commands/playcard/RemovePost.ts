...
export class Playcard {
  ...
  async remove(
    ...
    const regex = /(?:https:\/\/(?:canary\.)?discord\.com\/channels\/\d+\/\d+\/)?(?<messageId>\d+)/;
    const messageToFetch = msgToDelete.match(regex)?.groups?.messageId;
    if (!messageToFetch) return interaction.editReply(ErrorMessage.InvalidContent);
    ...
    if (!fetchedMessage) return interaction.editReply(ErrorMessage.UnknownMessage);
