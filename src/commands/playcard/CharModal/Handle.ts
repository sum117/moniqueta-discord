...
export class CharModal {
  ...
  private async _handleDiscussion(
    charId: number,
    ...
    await discussionChannel.send({
      content: `${
        userMention(authorId) + " " + userMention(interaction.user.id)
      }`,
      embeds: [discussEmbed],
    });
          ...
          () =>
            msg.deletable
              ? msg
                  .delete()
                  .catch(() =>
                    console.log(ErrorMessage.CouldNotDeleteUnknownMessage)
                  )
              : null,
          5000
        )
