import { ModalSubmitInteraction, SelectMenuInteraction, Modal, MessageEmbed, MessageActionRow, MessageButton, TextInputComponent } from "discord.js"
...
export default {
    ...
    description: 'Gera uma ficha de personagem para a avaliação pelos administradores.',
    /**
    ...
    async execute(interaction) {
        ...
        switch (interaction.type) {
            case "MESSAGE_COMPONENT":
                    ...
                    if (choices.size === 3) {
                        return interaction.showModal(
                            createForm([
                                ...
                                [, 'persoHabilidade', 'Habilidade', 'PARAGRAPH', 'A habilidade do personagem não irá interferir no combate.', 4000],
                                [, 'persoImagem', 'Link de Imagem', 'SHORT', 'Envie apenas links de imagem com a extensão no final, nada mais. Ex: https://i.imgur.com/image.png', 500]
                            ])
                        )
                    }
                    return handleChoice()
                ...
                break;
            case "MODAL_SUBMIT":
                if (customId === 'ficha') await interaction.reply({
                    ephemeral: true,
                    content: '✅ Ficha enviada com sucesso! Por favor, aguarde sua aprovação. Você será notificado caso haja algum problema. Com suas DMs abertas, seu chat comigo receberá uma cópia do os administradores estão vendo.',
                })
                const choices = sheet.get(user.id)
                ...
                const userInput = ((inputs = [['']]) => {
                    ...
                    return map
                })([['nome', 'persoNome'], ['personalidade', 'persoPersonalidade'], ['fisico', 'persoFisico'], ['habilidade', 'persoHabilidade'], ['imagem', 'persoImagem']])
                const embedArray = (() => {
                    const array = [
                        new MessageEmbed()
                            .setAuthor({
                            ...
                            .addField('Genero', title(choices.get('genero')) === "Masculino"
                                ? "♂️ Masculino"
                                : title(choices.get('genero')) === "Feminino"
                                    ? "♀️ Feminino"
                                    : "👽 Descubra", true
                            )
                ...
                canalDeAdmin.send({ content: `Ficha de ${user}`, embeds: embedArray, components: [components] })
                break;
        }
    }
}
